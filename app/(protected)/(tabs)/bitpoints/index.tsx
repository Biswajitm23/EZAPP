...
export default function CollabDaysScreen() {
  const query = useQuery({
    ...
    queryFn: ({ signal }) =>
      collaborationService.getCollaboration(undefined, signal),
    retry: false,
  });
  ...
  const days = useMemo<CollaborationDay[]>(
    () => query.data?.days ?? [],
    [query.data],
  );
  ...
  const onEndReached = useCallback(() => {
    setVisible((v) =>
      v < days.length ? Math.min(v + PAGE_SIZE, days.length) : v,
    );
  }, [days.length]);
  ...
  const renderHeader = () => (
    <>
      <Reveal index={0}>
        <AppHeader showBack kicker="" title="Collab Days" />
      </Reveal>
      ...
      <Reveal index={1}>
        ...
        >
          {/* {!!query.data?.title && (
            <Text style={styles.subtitle}>{query.data.title}</Text>
          )} */}
          <View style={styles.heroTop}>
            <Text style={styles.heroLabel}>
              {heroDay ? "Your Next Collab Day" : "Upcoming Collab Day"}
            </Text>
          ...
          {heroDay ? (
            <>
              <Text style={styles.heroDate}>
                {prettyDate(heroDay.date, heroDay.weekday)}
              </Text>
              <Text style={styles.heroStatus}>{heroDay.status === "Working Day" ? "In Office Day" : "Work From Home Day"}</Text>
            ...
            <>
              ...
              <Text style={styles.heroStatus}>
                No further collab days this period.
              </Text>
            </>
          )}
        </LinearGradient>
        ...
        <View style={styles.listHead}>
          ...
          {query.data?.total_office_days != null && (
            <Text style={styles.countPill}>
              {query.data.total_office_days} days
            </Text>
          )}
        </View>
      </Reveal>
      ...
      <Reveal index={Math.min(index, 6)}>
        <View style={[styles.row, isActive && styles.rowActive]}>
          <View style={[styles.dateChip, isActive && styles.dateChipActive]}>
            ...
            <Text
              style={[styles.dateChipWk, isActive && styles.dateChipWkActive]}
            >
              {item.weekday}
            </Text>
          </View>
